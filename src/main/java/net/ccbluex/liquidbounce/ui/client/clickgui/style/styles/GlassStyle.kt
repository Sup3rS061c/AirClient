...
object GlassStyle : Style() {
    private val glassBackground = Color(255, 255, 255, 180)
    private val glassHeader = Color(255, 255, 255, 200)
    private val glassBorder = Color(255, 255, 255, 100)
    private val glassShadow = Color(0, 0, 0, 50)
    
    private fun drawGlassPanel(x: Int, y: Int, x2: Int, y2: Int, accentColor: Color) {
        for (i in 5 downTo 1) {
            val shadowAlpha = (50 * (1F - i / 5F)).toInt()
            drawRect(x - i, y + i, x2 + i, y2 + i, Color(0, 0, 0, shadowAlpha).rgb)
        }
        
        drawRect(x, y, x2, y2, glassBackground.rgb)
        
        drawBorderedRect(x, y, x2, y2, 1, glassBorder.rgb, glassBackground.rgb)
        
        drawRect(x, y, x2, y + 20, glassHeader.rgb)
        
        drawRect(x + 2, y + 18, x2 - 2, y + 19, accentColor.rgb)
    }
    
    private fun drawGlassSlider(x: Float, y: Float, width: Float, height: Float, progress: Float, color: Color) {
        drawRect(x, y, x + width, y + height, Color(255, 255, 255, 100).rgb)
        
        val sliderWidth = width * progress
        drawRect(x, y, x + sliderWidth, y + height, Color(color.red, color.green, color.blue, 150).rgb)
        
        val handleX = x + sliderWidth - 4
        drawRect(handleX, y - 2, handleX + 8, y + height + 2, Color(255, 255, 255, 230).rgb)
        drawBorderedRect(handleX, y - 2, handleX + 8, y + height + 2, 1F, color.rgb, Color(255, 255, 255, 230).rgb)
    }
    
    private fun drawGlassToggle(x: Float, y: Float, width: Float, height: Float, enabled: Boolean, color: Color) {
        val trackColor = if (enabled) Color(color.red, color.green, color.blue, 150) else Color(200, 200, 200, 150)
        drawRect(x, y, x + width, y + height, trackColor.rgb)
        drawBorderedRect(x, y, x + width, y + height, 1F, Color(255, 255, 255, 100).rgb, trackColor.rgb)
        
        val handleSize = height - 4
        val handleX = if (enabled) x + width - handleSize - 2 else x + 2
        drawRect(handleX, y + 2, handleX + handleSize, y + 2 + handleSize, Color(255, 255, 255, 230).rgb)
    }
    
    private fun drawGlassCheckbox(x: Float, y: Float, size: Float, checked: Boolean, color: Color) {
        drawRect(x, y, x + size, y + size, Color(255, 255, 255, 150).rgb)
        drawBorderedRect(x, y, x + size, y + size, 1F, Color(255, 255, 255, 100).rgb, Color(255, 255, 255, 150).rgb)
        
        if (checked) {
            drawRect(x + 2, y + 2, x + size - 2, y + size - 2, Color(color.red, color.green, color.blue, 200).rgb)
        }
    }
    ...
    override fun drawPanel(mouseX: Int, mouseY: Int, panel: Panel) {
        ...
        drawGlassPanel(panel.x, panel.y, panel.x + panel.width, panel.y + panel.height + panel.fade, accentColor)
        
        val xPos = panel.x - (fontSemibold35.getStringWidth(StringUtils.stripControlCodes(panel.name)) - 100) / 2
        fontSemibold35.drawString(panel.name, xPos, panel.y + 7, Color(60, 60, 65).rgb)

        if (panel.scrollbar && panel.fade > 0) {
            drawRect(panel.x - 2, panel.y + 21, panel.x, panel.y + 16 + panel.fade, Color(255, 255, 255, 100).rgb)
    ...
    override fun drawHoverText(mouseX: Int, mouseY: Int, text: String) {
        ...
        for (i in 3 downTo 1) {
            val shadowAlpha = (30 * (1F - i / 3F)).toInt()
            drawRect(x + 9 - i, y + i, x + width + i, y + height + i, Color(0, 0, 0, shadowAlpha).rgb)
        }
        
        drawRect(x + 9, y, x + width, y + height, glassBackground.rgb)
        drawBorderedRect(x + 9, y, x + width, y + height, 1, glassBorder.rgb, glassBackground.rgb)
        
        lines.forEachIndexed { index, text ->
            fontSemibold35.drawString(text, x + 12, y + 3 + (fontSemibold35.fontHeight) * index, Color(60, 60, 65).rgb)
        }
    }
    ...
    override fun drawModuleElementAndClick(
        ...
        if (isHovered) {
            drawRect(moduleElement.x, moduleElement.y, moduleElement.x + moduleElement.width, 
                    moduleElement.y + moduleElement.height, Color(255, 255, 255, 80).rgb)
        }
        ...
        if (moduleValues.isNotEmpty()) {
            ...
            if (moduleElement.showSettings) {
                ...
                if (moduleElement.settingsWidth > 0 && moduleElement.settingsHeight > 0) {
                    for (i in 4 downTo 1) {
                        val shadowAlpha = (40 * (1F - i / 4F)).toInt()
                        drawRect(minX - i, yPos + i, maxX + i, yPos + moduleElement.settingsHeight + i, Color(0, 0, 0, shadowAlpha).rgb)
                    }
                    drawRect(minX, yPos, maxX, yPos + moduleElement.settingsHeight, glassBackground.rgb)
                    drawBorderedRect(minX, yPos, maxX, yPos + moduleElement.settingsHeight, 1, glassBorder.rgb, glassBackground.rgb)
                }
                ...
                for (value in moduleValues) {
                    ...
                    when (value) {
                        is BoolValue -> {
                            ...
                            fontSemibold35.drawString(text, minX + 2, yPos + 5, Color(60, 60, 65).rgb)
                            drawGlassToggle(maxX.toFloat() - 30, yPos + 3F, 26F, 12F, value.get(), accentColor)
                        ...
                        is ListValue -> {
                            ...
                            for (valueOfList in value.values) {
                                ...
                                if (value.openList) {
                                    ...
                                    if (selected) {
                                        drawGlassCheckbox(minX.toFloat() + 2, yPos + 3F, 10F, true, accentColor)
                                    }
                                    ...
                                    fontSemibold35.drawString(
                                        ...
                                        minX + 16,
                                        yPos + 5,
                                        if (selected) accentColor.rgb else Color(100, 100, 105).rgb
                                    )
                        ...
                        is FloatValue -> {
                            ...
                            if (mouseButton == 0 && mouseX in minX..maxX && mouseY in yPos + 15..yPos + 25 || sliderValueHeld == value) {
                            ...
                            fontSemibold35.drawString(text, minX + 2, yPos + 4, Color(60, 60, 65).rgb)
                            
                            val displayValue = value.get().coerceIn(value.range)
                            val progress = (displayValue - value.minimum) / (value.maximum - value.minimum)
                            drawGlassSlider(minX.toFloat() + 4, yPos + 18F, maxX.toFloat() - minX - 8F, 6F, progress, accentColor)
                        ...
                        is IntRangeValue -> {
                            ...
                            if (mouseButton == 0 && mouseX in startX..endX && mouseY in startY - 2..startY + 11 || sliderValueHeld == value) {
                                val leftSliderPos =
                            ...
                            drawRect(minX + 4, yPos + 18, maxX - 4, yPos + 24, Color(255, 255, 255, 100).rgb)
                            ...
                            drawRect(sliderX1 - 4, yPos + 16F, sliderX1 + 4, yPos + 26F, Color(255, 255, 255, 230).rgb)
                            drawBorderedRect(sliderX1 - 4, yPos + 16F, sliderX1 + 4, yPos + 26F, 1F, accentColor.rgb, Color(255, 255, 255, 230).rgb)
                            drawRect(sliderX2 - 4, yPos + 16F, sliderX2 + 4, yPos + 26F, Color(255, 255, 255, 230).rgb)
                            drawBorderedRect(sliderX2 - 4, yPos + 16F, sliderX2 + 4, yPos + 26F, 1F, accentColor.rgb, Color(255, 255, 255, 230).rgb)
                        ...
                        is FontValue -> {
                            ...
                            fontSemibold35.drawString(displayString, minX + 2, yPos + 5, Color(60, 60, 65).rgb)
                        ...
                        is ColorValue -> {
                            ...
                            chosenText?.let {
                                ...
                                highlightCursor = {
                                    ...
                                    drawRect(cursorX, cursorY - 3F, cursorX + 1F, cursorY + fontSemibold35.fontHeight - 2, Color(60, 60, 65).rgb)
                                }
                            }
                            ...
                            if (value.showOptions) {
                                ...
                                rgbaLabels.forEachIndexed { index, label ->
                                    ...
                                    val defaultColor = if (isEmpty) Color(150, 150, 155) else accentColor
                                    val defaultText = if (isEmpty) "($rgbaValueText)" else rgbaValueText

                                    fontSemibold35.drawString(label, textX, yPosition, Color(60, 60, 65).rgb)
                            ...
                            fontSemibold35.drawString(combinedText, textX, textY, Color(60, 60, 65).rgb)
                            highlightCursor()

                            val normalBorderColor = if (rainbow) glassBorder else accentColor
                            val rainbowBorderColor = if (rainbow) accentColor else glassBorder
                            ...
                            chosenText?.let {
                                ...
                                highlightCursor = { textX ->
                                    ...
                                    drawRect(cursorX, textY - 3, cursorX + 1, textY + fontSemibold35.fontHeight - 2, Color(60, 60, 65).rgb)
                                }
                            }

                            fontSemibold35.drawString(startText, startX, textY, Color(60, 60, 65).rgb)

                            val defaultColor = if (shouldPushToRight) Color(150, 150, 155) else accentColor
