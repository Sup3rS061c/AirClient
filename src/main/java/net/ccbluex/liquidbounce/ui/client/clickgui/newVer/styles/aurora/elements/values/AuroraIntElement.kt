...
class AuroraIntElement(value: IntValue) : AuroraValueElement<Int>(value) {
    private var dragging = false
    private var animProgress = 0F

    override fun drawElement(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, accentColor: Color): Float {
        val intValue = value as IntValue
        valueHeight = 40F

        val displayValue = intValue.get().coerceIn(intValue.range)
        val progress = (displayValue - intValue.minimum).toFloat() / (intValue.maximum - intValue.minimum)

        animProgress += (progress - animProgress) * 0.15F

        val text = "${intValue.name}: ${intValue.get()}${intValue.suffix ?: ""}"
    ...
    private fun updateValue(mouseX: Int, x: Float, width: Float) {
        ...
        val newValue = intValue.minimum + ((intValue.maximum - intValue.minimum) * newProgress).toInt()
        intValue.set(newValue.coerceIn(intValue.range))
    }
}
